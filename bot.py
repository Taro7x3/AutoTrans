...
def build_system_prompt(direction: str, dictionary: dict) -> str:
    ...
    if direction == "ja_to_ko":
        ...
        return (
            ...
            "以下のルールを厳守してください：\n"
            "1. ゲームのコール（敵の位置やUlt報告など）は短く迅速に伝える表現にする\n"
            "2. 以下の専門用語マッピングを優先的に使用する："
            f"{term_section}\n"
            "3. 翻訳結果のみを出力し、説明や注釈は一切付けない\n"
            "4. 原文のニュアンスと緊急感を保つ"
            "5. 雑談や感想、日常会話は省略せず、話し手のニュアンスや感情を保ったより自然かつ話し手の内容を忠実に翻訳した口語（パンマル/タメ口）にする\n"
        )
...
class VADSink(discord.sinks.Sink):
    ...
    def write(self, data: "VoiceData", user) -> None:
        ...
        """
        # ── 診断: write()呼び出しカウンター（patches/_diag_counters に記録） ──
        try:
...
async def translate_with_ollama(
    ...
    payload = {
        ...
        "stream": False,
        "options": {
            "temperature": 0.2,  # 翻訳の一貫性を高めるため低めに設定
            "top_p": 0.8,             # 低確率の異常トークンを排除
            "num_predict": 200,  # ゲームコールは短いので上限を設定
            "repeat_penalty": 1.15,   # 同一フレーズ・記号の無限ループを防止
        },
    }
...
def transcribe_audio(audio_bytes: bytes, sample_rate: int) -> tuple[str, str]:
    ...
    segments, info = whisper_model.transcribe(
        ...
        initial_prompt="こんにちは、ありがとう。ナイス、ヒール、敵、Ult、裏。안녕하세요, 감사합니다. 나이스, 힐, 적, 궁, 뒤.",
    )
