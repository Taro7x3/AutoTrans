...
スラッシュコマンドを即時反映させるために必要です。

1. Discordの設定 → 「詳細設定」→「開発者モード」をON（上記と同じ）
2. Botを招待したサーバーのサーバー名（左サイドバー）を**右クリック**
3. **「IDをコピー」** をクリック

> **なぜ必要か:** `GUILD_ID` を設定するとスラッシュコマンドがギルドコマンドとして登録され、**即時反映**されます。未設定の場合はグローバルコマンドとして登録され、Discordへの反映に**最大1時間**かかります。
...
`DISCORD_TOKEN`、`TEXT_CHANNEL_ID`、`GUILD_ID` を実際の値に書き換えて保存してください。

| 変数 | 取得方法 | 必須 |
|------|----------|------|
| `DISCORD_TOKEN` | Developer Portal → Bot → Token → Reset Token | ✅ 必須 |
| `TEXT_CHANNEL_ID` | 開発者モードON → テキストチャンネル右クリック → IDをコピー | ✅ 必須 |
| `GUILD_ID` | 開発者モードON → サーバー名右クリック → IDをコピー | 推奨（未設定時はコマンド反映に最大1時間） |
...
1. Botを招待する際に `applications.commands` スコープが含まれているか確認してください。
2. `.env` に `GUILD_ID` を設定してください（サーバー名右クリック → IDをコピー）。`GUILD_ID` を設定するとギルドコマンドとして**即時反映**されます。
3. `GUILD_ID` を設定せずグローバルコマンドとして登録した場合、反映まで最大1時間かかります。
